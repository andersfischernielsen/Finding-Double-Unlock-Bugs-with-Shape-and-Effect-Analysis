...
\todo{Undefined behaviour is ... - this entirely paragraph has too long sentences with broken flow.  Read it fluently (relatively fast) aloud - you will see the fixes right away (at the points you stumble naturally).}
...
\noindent This report will detail my approach for detecting such double unlock errors based on previous work, give an overview of the concrete implementation of my work and finally evaluate the results of validating an assembled set of files known to have bugs in the Linux kernel components.
\todo{It would be good to add one more paragraph (or rewrite the last one) to more formally summarize the contents of the paper.  "The report proceeds as follows.  In Section 2,  I present the background about double unlock bugs and the EBA bug analyzer.  In Section  3, I present the design and implementation of the double unlock bug finder, the key outcome of this work. Section 4 evaluates the efficiency of my bug finder. In Section 5 ..." (you get the drill)}